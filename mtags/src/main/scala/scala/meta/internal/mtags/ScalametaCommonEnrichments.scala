  ...
  implicit class XtensionAbsolutePath(path: AbsolutePath) {
    ...
    private class MetalsVisitor extends FileVisitor[Path] {
      ...
      override def visitFileFailed(
        ...
        logger.error(s"Failed to visit file $file", exc)
    ...
    def isTwirlTemplate: Boolean = {
      filename.endsWith(".scala.html") || filename.endsWith(".scala.js") ||
      filename.endsWith(".scala.xml") || filename.endsWith(".scala.txt")
    }
